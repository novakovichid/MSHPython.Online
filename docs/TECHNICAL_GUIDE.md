...
- `projects`: `{ projectId, title, files, assets, lastActiveFile, updatedAt }`
  - **⚠️ DEPRECATED**: Поле `assets` больше не используется и заморожено. Использование Assets отключено.
- `blobs`: `{ blobId, data }`
...
- **⚠️ DEPRECATED**: Логика работы с ресурсами (assets) оставлена для обратной совместимости, но не используется.
...
- **⚠️ DEPRECATED**: Assets (ресурсы) больше не поддерживаются и заморожены.

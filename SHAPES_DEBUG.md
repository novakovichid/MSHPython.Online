...
Skull'пт - это интерпретатор Python, скомпилированный в JavaScript. Его встроенная библиотека turtle графики имеет ограничения:

1. **Shape класс не поддерживает изображения по имени файла**
   - `Shape("image", "image.png")` создаёт объект, но не загружает изображение
   - Skull'пт не знает как получить URL или данные файла
...
3. **Это работает на trinket.io потому что:**
   - Trinket.io использует другую конфигурацию/версию Skull'пт'а
...
- Преобразовать в что-то, что Skull'пт может использовать
...
## Выводы
Это не ошибка в коде, а архитектурное ограничение Skull'пт'а.
Для полной поддержки изображений нужна либо:
- Новая версия Skull'пт'а с поддержкой
